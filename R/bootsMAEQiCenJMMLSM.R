...
bootsMAEQiCenJMMLSM <- function(seed = 100, n = 2000, landmark.time = 4,
                                horizon.time = 5, increment = 0.3, exact.obs = 0) {
  
  data <- SimJMdata(seed = 99+799, n = n,
                    nc = 100,
  ...
  iCenfit <- iCenJMMLSM(Ydata = Ydata, Tdata = Tdata,
                        ...
                        quadpoint = 15, print.para = FALSE,
                        initial.para = TRUE,
                        pStol = 1e-06, c = 0.95,
                        hazard.kernel = "Epanechnikov")
  
  MAEQ <- MAEQiCenJMMLSM(seed = seed, iCenfit, landmark.time = landmark.time, horizon.time = horizon.time, obs.time = "Oij")
  iCenRES <- summary(MAEQ, digits = 4)
  
  midpoint.fit <- MAEQmidpointJMMLSM(seed = seed, 
                                     Ydata = Ydata,
                                     Tdata = Tdata,
                                     Sdata = Sdata,
                                     long.formula = Yij ~ X11 + X12, 
                                     ...
                                     obs.time = "Oij", 
                                     datatype = "midpoint",
                                     quadpoint = 15, maxiter = 1000, n.cv = 3, 
                                     quantile.width = 0.25)
  
  midpointRES <- summary(midpoint.fit, digits = 4) 
  
  result <- list(iCenRES = iCenRES, midpointRES = midpointRES)
