...
//
// [[Rcpp::export]]
int getEC(const Eigen::VectorXd & beta, const Eigen::VectorXd & tau, 
    ...
    for (t=0;t<nt(j);t++)
    {
      ...
      if (pSLR(countt, 3) <= pStol || isnan(pSLR(countt, 3))) {
        ...
        for (db=0;db<point;db++)
        {
          ...
          if(status(countt)==1)  temp*=haz0*exp(xgamma+MultVV(alpha, ri));
          temp*=exp(0-cuh0*exp(xgamma+MultVV(alpha, ri)));
          temp*=pSLR(countt, 3);
...
// [[Rcpp::export]]
int getECad(const Eigen::VectorXd & beta, const Eigen::VectorXd & tau, 
            const Eigen::VectorXd & gamma, const Eigen::VectorXd & alpha,  
            const Eigen::MatrixXd & H0Y, const Eigen::MatrixXd & Sig, 
            const Eigen::MatrixXd & X1, const Eigen::MatrixXd & Z, 
            const Eigen::MatrixXd & W, const Eigen::VectorXd & Y, 
  ...
  for(j=0;j<k;j++)
  {
    ...
    for (t=0;t<nt(j);t++)
    {
      ...
      if (pSLR(countt, 3) <= pStol || isnan(pSLR(countt, 3))) {
        ...
        for (i=0;i<(p1a+1);i++) Hi.row(i) = Poscov.row(countt*(p1a+1)+i);
